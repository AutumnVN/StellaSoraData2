local ActivityWinterNightCtrl = class("ActivityWinterNightCtrl", BaseCtrl)
local TimerManager = require("GameCore.Timer.TimerManager")
local ClientManager = CS.ClientManager.Instance
ActivityWinterNightCtrl._mapNodeConfig = {
	btnGo = {
...
function ActivityWinterNightCtrl:Awake()
	self.tbGridCtrl = {}
end
function ActivityWinterNightCtrl:OnDisable()
	self:UnbindCtrl()
end
function ActivityWinterNightCtrl:InitActData(actData)
...
function ActivityWinterNightCtrl:UnInit()
	self:UnbindCtrl()
end
function ActivityWinterNightCtrl:UnbindCtrl()
...
function ActivityWinterNightCtrl:RefreshLockState()
...
function ActivityWinterNightCtrl:RefreshTimeout()
...
function ActivityWinterNightCtrl:RefreshDate()
...
function ActivityWinterNightCtrl:RefreshReward()
...
function ActivityWinterNightCtrl:RefreshRewardGridItem(go, index)
	local actGroupCfg = self.actData:GetActGroupCfgData()
	local rewardData = actGroupCfg.RewardsShow
...
function ActivityWinterNightCtrl:BtnRewardGridClick(goGrid, gridIndex)
...
function ActivityWinterNightCtrl:OnBtnClick_Go()
	local actGroupCfg = self.actData:GetActGroupCfgData()
	if actGroupCfg ~= nil and actGroupCfg ~= nil then
		if actGroupCfg.TransitionId ~= nil and actGroupCfg.TransitionId > 0 then
			local callback = function()
				EventManager.Hit(EventId.OpenPanel, PanelId.WinterNightThemePanel, actGroupCfg.Id, true)
			end
...
function ActivityWinterNightCtrl:OnBtnClick_Detail()
	local actGroupCfg = self.actData:GetActGroupCfgData()
...
function ActivityWinterNightCtrl:ClearActivity()
end
return ActivityWinterNightCtrl
