...
local eightLvGroupPadding = {98, 98}
local eightLvGroupPaddingOpen = {98, 818}
local fourLvGroupPadding = {98, 98}
local fourLvGroupPaddingOpen = {98, 910}
ActivityLevelsSelectCtrl._mapNodeConfig = {
	...
	togRoot = {
		nCount = 8,
		sCtrlName = "Game.UI.ActivityTheme.10105.ActivityLevels.ActivityLevelsLvCtrl"
	},
	rt_Toggle = {},
...
function ActivityLevelsSelectCtrl:RefreshTogList(nType, nDifficulty)
	...
	self._mapNode.rtToggles.gameObject:SetActive(self.currentTypeLvCount == 8)
	...
	if isEightLv then
		if self.isOpenLvMsg == false then
			NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_Toggle, eightLvGroupPadding[1], eightLvGroupPadding[2], 0, 0)
		else
			NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_Toggle, eightLvGroupPaddingOpen[1], eightLvGroupPaddingOpen[2], 0, 0)
		end
	elseif self.isOpenLvMsg == false then
		NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_ToggleFour, fourLvGroupPadding[1], fourLvGroupPadding[2], 0, 0)
	else
		NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_ToggleFour, fourLvGroupPaddingOpen[1], fourLvGroupPaddingOpen[2], 0, 0)
	end
	for i = 1, self.currentTypeLvCount do
		do
			...
			if isEightLv then
				...
				self._mapNode.togRootFour[i]:InitData(self, nType, self.activityLevelsData, tmpData, isOpen, isLevelUnLock)
			end
...
function ActivityLevelsSelectCtrl:ShowBossInfo(isShow)
	...
	if not isShow then
		if self.currentTypeLvCount == 8 then
			NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_Toggle, eightLvGroupPadding[1], eightLvGroupPadding[2], 0, 0)
		else
			NovaAPI.SetLayoutGroupPadding(self._mapNode.rt_ToggleFour, fourLvGroupPadding[1], fourLvGroupPadding[2], 0, 0)
		end
...
function ActivityLevelsSelectCtrl:RefreshInstanceInfo(nType, nHard, isInit)
	if self.currentTypeLvCount == 8 then
		...
		local wait = function()
			...
			elseif 7 < nHard then
				self._mapNode.rt_ToggleTrans:DOLocalMoveX(-1120, moveTime)
			else
				self._mapNode.rt_ToggleTrans:DOLocalMoveX(-300 * (nHard - 4), moveTime)
			end
		end
		cs_coroutine.start(wait)
	else
		...
		local wait = function()
			...
			elseif 3 < nHard then
				self._mapNode.rt_ToggleTransFour:DOLocalMoveX(-710, moveTime)
			else
				self._mapNode.rt_ToggleTransFour:DOLocalMoveX(-300, moveTime)
			end
		end
		cs_coroutine.start(wait)
	end
