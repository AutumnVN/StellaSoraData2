...
function ActivityShopGoodsCtrl:OnGridRefresh(goGrid, gridIndex)
	...
	if not self.tbGridCtrl[nInstanceID] then
		local actGroupId = ConfigTable.GetData("Activity", self._panel.nActId).MidGroupId
		self.tbGridCtrl[nInstanceID] = self:BindCtrlByNode(goGrid, "Game.UI.ActivityTheme." .. actGroupId .. ".Shop.ActivityShopGoodsItemCtrl")
	end
...
function ActivityShopGoodsCtrl:OnGridBtnClick(goGrid, gridIndex)
	local nIndex = gridIndex + 1
	local mapData = self.tbGoods[nIndex]
	EventManager.Hit(EventId.OpenPanel, PanelId.ActivityShopPopup, mapData, self.nShopId, self._panel.nActId)
end
