local ChristmasTaskCtrl = class("ChristmasTaskCtrl", BaseCtrl)
...
ChristmasTaskCtrl._mapNodeConfig = {
	TopBarPanel = {
...
function ChristmasTaskCtrl:BuildData(nActivityId)
...
function ChristmasTaskCtrl:onGridRefresh_Tab(go)
	...
	if bInActGroup == false then
		RedDotManager.RegisterNode(RedDotDefine.Activity_Group_Task_Group, {
			self.nActivityId,
			mapData.nGroupId
		}, goRedDot_on, nil, nil, true)
		RedDotManager.RegisterNode(RedDotDefine.Activity_Group_Task_Group, {
			self.nActivityId,
			mapData.nGroupId
		}, goRedDot_off, nil, nil, true)
	else
		RedDotManager.RegisterNode(RedDotDefine.Activity_Group_Task_Group, {
			nActGroupId,
			self.nActivityId,
...
function ChristmasTaskCtrl:onGridBtnClick_Tab(go)
	local nIndex = tonumber(go.name) + 1
...
function ChristmasTaskCtrl:RefreshScaleOnClick_State(tr, nIndex, mapCfgData_ActivityTaskGroup, sProgress)
...
function ChristmasTaskCtrl:refresh_Task(bPlayAnim)
...
function ChristmasTaskCtrl:onGridRefresh_Task(go)
	local nIndex = tonumber(go.name) + 1
	local mapData = self.tbData[self.nCurGroupIndex]
...
function ChristmasTaskCtrl:onEvent_ClickTaskJump(goBtn)
	local nJumpId = tonumber(goBtn.transform:GetChild(0).name)
	if 0 < nJumpId then
		JumpUtil.JumpTo(nJumpId)
	end
end
function ChristmasTaskCtrl:refresh_Group()
	local mapData = self.tbData[self.nCurGroupIndex]
...
function ChristmasTaskCtrl:onGridRefresh_GroupRewardItem(go)
	local nIndex = tonumber(go.name) + 1
...
function ChristmasTaskCtrl:onGridBtnClick_GroupRewardItem(go)
...
function ChristmasTaskCtrl:onBtn_GroupDone()
	local mapData = self.tbData[self.nCurGroupIndex]
	local cb = function()
		self:BuildData(self.nActivityId)
		self:refresh_Tab()
