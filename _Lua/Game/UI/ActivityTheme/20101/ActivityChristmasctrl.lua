local ActivityChristmasCtrl = class("ActivityChristmasCtrl", BaseCtrl)
local TimerManager = require("GameCore.Timer.TimerManager")
local ClientManager = CS.ClientManager.Instance
ActivityChristmasCtrl._mapNodeConfig = {
	btnGo = {
...
function ActivityChristmasCtrl:Awake()
	self.tbGridCtrl = {}
end
function ActivityChristmasCtrl:OnDisable()
	self:UnbindCtrl()
end
function ActivityChristmasCtrl:InitActData(actData)
...
function ActivityChristmasCtrl:UnInit()
	self:UnbindCtrl()
end
function ActivityChristmasCtrl:UnbindCtrl()
...
function ActivityChristmasCtrl:RefreshLockState()
...
function ActivityChristmasCtrl:RefreshTimeout()
...
function ActivityChristmasCtrl:RefreshDate()
...
function ActivityChristmasCtrl:RefreshReward()
...
function ActivityChristmasCtrl:RefreshRewardGridItem(go, index)
	local actGroupCfg = self.actData:GetActGroupCfgData()
	local rewardData = actGroupCfg.RewardsShow
...
function ActivityChristmasCtrl:BtnRewardGridClick(goGrid, gridIndex)
...
function ActivityChristmasCtrl:OnBtnClick_Go()
	local actGroupCfg = self.actData:GetActGroupCfgData()
	if actGroupCfg ~= nil and actGroupCfg ~= nil then
		if actGroupCfg.TransitionId ~= nil and actGroupCfg.TransitionId > 0 then
			local callback = function()
				EventManager.Hit(EventId.OpenPanel, PanelId.ChristmasThemePanel, actGroupCfg.Id)
			end
...
function ActivityChristmasCtrl:OnBtnClick_Detail()
	local actGroupCfg = self.actData:GetActGroupCfgData()
...
function ActivityChristmasCtrl:ClearActivity()
end
return ActivityChristmasCtrl
