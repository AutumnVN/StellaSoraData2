local ActivityPostalCtrl = class("ActivityPostalCtrl", BaseCtrl)
local TimerManager = require("GameCore.Timer.TimerManager")
local ClientManager = CS.ClientManager.Instance
ActivityPostalCtrl._mapNodeConfig = {
	btnGo = {
...
function ActivityPostalCtrl:Awake()
	self.tbGridCtrl = {}
end
function ActivityPostalCtrl:OnDisable()
	self:UnbindCtrl()
end
function ActivityPostalCtrl:InitActData(actData)
...
function ActivityPostalCtrl:UnInit()
	self:UnbindCtrl()
end
function ActivityPostalCtrl:UnbindCtrl()
...
function ActivityPostalCtrl:RefreshLockState()
...
function ActivityPostalCtrl:RefreshTimeout()
...
function ActivityPostalCtrl:RefreshDate()
...
function ActivityPostalCtrl:RefreshReward()
...
function ActivityPostalCtrl:RefreshRewardGridItem(go, index)
	local actGroupCfg = self.actData:GetActGroupCfgData()
	local rewardData = actGroupCfg.RewardsShow
...
function ActivityPostalCtrl:BtnRewardGridClick(goGrid, gridIndex)
...
function ActivityPostalCtrl:OnBtnClick_Go()
	local actGroupCfg = self.actData:GetActGroupCfgData()
	if actGroupCfg ~= nil and actGroupCfg ~= nil then
		if actGroupCfg.TransitionId ~= nil and actGroupCfg.TransitionId > 0 then
			local callback = function()
				EventManager.Hit(EventId.OpenPanel, PanelId.PostalThemePanel, actGroupCfg.Id, true)
			end
...
function ActivityPostalCtrl:OnBtnClick_Detail()
	local actGroupCfg = self.actData:GetActGroupCfgData()
...
function ActivityPostalCtrl:ClearActivity()
end
return ActivityPostalCtrl
