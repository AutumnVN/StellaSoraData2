local PostalTaskCtrl = class("PostalTaskCtrl", BaseCtrl)
...
function PostalTaskCtrl:FadeIn()
	EventManager.Hit(EventId.SetTransition)
end
function PostalTaskCtrl:BuildData(nActivityId)
...
function PostalTaskCtrl:onGridRefresh_Tab(go)
...
function PostalTaskCtrl:onGridBtnClick_Tab(go)
	local nIndex = tonumber(go.name) + 1
...
function PostalTaskCtrl:RefreshScaleOnClick_State(tr, nIndex, mapCfgData_ActivityTaskGroup, sProgress)
	local canvasGroupOn = tr:Find("scale_on_click/imgDb_on"):GetComponent("CanvasGroup")
	local canvasGroupOff = tr:Find("scale_on_click/imgDb_off"):GetComponent("CanvasGroup")
	if self.nCurGroupIndex == nIndex then
...
function PostalTaskCtrl:refresh_Task(bPlayAnim)
...
function PostalTaskCtrl:onGridRefresh_Task(go)
	local nIndex = tonumber(go.name) + 1
	local mapData = self.tbData[self.nCurGroupIndex]
...
function PostalTaskCtrl:onEvent_ClickTaskJump(goBtn)
	local nJumpId = tonumber(goBtn.transform:GetChild(0).name)
	if 0 < nJumpId then
		JumpUtil.JumpTo(nJumpId)
	end
end
function PostalTaskCtrl:refresh_Group()
	local mapData = self.tbData[self.nCurGroupIndex]
...
function PostalTaskCtrl:onGridRefresh_GroupRewardItem(go)
	local nIndex = tonumber(go.name) + 1
...
function PostalTaskCtrl:onGridBtnClick_GroupRewardItem(go)
...
function PostalTaskCtrl:onBtn_GroupDone()
	local mapData = self.tbData[self.nCurGroupIndex]
	local cb = function()
		self:BuildData(self.nActivityId)
		self:refresh_Tab()
