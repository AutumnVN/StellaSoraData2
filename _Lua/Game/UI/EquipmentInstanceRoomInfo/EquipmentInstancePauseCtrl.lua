local EquipmentInstancePauseCtrl = class("EquipmentInstancePauseCtrl", BaseCtrl)
local GameCameraStackManager = CS.GameCameraStackManager.Instance
local AdventureModuleHelper = CS.AdventureModuleHelper
local GamepadUIManager = require("GameCore.Module.GamepadUIManager")
EquipmentInstancePauseCtrl._mapNodeConfig = {
	goBlur = {
...
EquipmentInstancePauseCtrl._mapEventConfig = {
	Equipment_Instance_Gameplay_Time = "OnEvent_Time",
	OpenEquipmentInstancePause = "Pause",
	GamepadUIReopen = "OnEvent_Reopen"
}
function EquipmentInstancePauseCtrl:Refresh()
	local mapCfg = ConfigTable.GetData("CharGemInstance", self.nLevelId)
	if mapCfg == nil then
		return
	end
	local curStar = PlayerData.EquipmentInstance:GetEquipmentInstanceStar(self.nLevelId)
	local tbCond = {}
	...
	for i = 1, 3 do
		...
		if cond == nil then
			...
			local _floorData = ConfigTable.GetData("CharGemInstanceFloor", mapCfg.FloorId)
			if _floorData then
				if cond[1] == 1 then
...
function EquipmentInstancePauseCtrl:PlayInAni()
...
function EquipmentInstancePauseCtrl:PlayCloseAni(bGiveUp)
	self._mapNode.aniWindow:Play("t_window_04_t_out")
	self._mapNode.aniBlur:SetTrigger("tOut")
	EventManager.Hit(EventId.TemporaryBlockInput, 0.2)
	self:AddTimer(1, 0.2, "OnPanelClose", true, true, true, bGiveUp)
end
function EquipmentInstancePauseCtrl:OnPanelClose(_, bGiveUp)
	PanelManager.InputEnable()
	GamepadUIManager.DisableGamepadUI("EquipmentInstancePauseCtrl")
...
function EquipmentInstancePauseCtrl:Awake()
	self._mapNode.safeAreaRoot:SetActive(false)
...
function EquipmentInstancePauseCtrl:OnEnable()
end
function EquipmentInstancePauseCtrl:OnDisable()
end
function EquipmentInstancePauseCtrl:OnDestroy()
end
function EquipmentInstancePauseCtrl:Pause(nLevelId, tbCharId)
	...
	self:PlayInAni()
	self:Refresh()
	GamepadUIManager.EnableGamepadUI("EquipmentInstancePauseCtrl", self.tbGamepadUINode)
end
function EquipmentInstancePauseCtrl:OnEvent_Time(nTime)
...
function EquipmentInstancePauseCtrl:OnBtnClick_GiveUp(btn)
	local sTip = ""
	local mapCfg = ConfigTable.GetData("CharGemInstance", self.nLevelId)
	if mapCfg == nil then
		printError("没有当前关卡信息")
...
function EquipmentInstancePauseCtrl:OnBtnClick_Close(btn)
	self:PlayCloseAni(false)
end
function EquipmentInstancePauseCtrl:OnBtnClick_Char(btn, nIndex)
end
function EquipmentInstancePauseCtrl:OnBtnClick_Skill(btn)
	EventManager.Hit(EventId.OpenPanel, PanelId.PopupSkillPanel, self.tbChar)
	self._mapNode.ActionBar.gameObject:SetActive(false)
end
function EquipmentInstancePauseCtrl:OnBtnClick_Settings(btn)
	EventManager.Hit(EventId.OpenPanel, PanelId.BattleSettings)
	self._mapNode.ActionBar.gameObject:SetActive(false)
end
function EquipmentInstancePauseCtrl:OnEvent_Reopen(sName)
	if sName ~= "EquipmentInstancePauseCtrl" then
		return
	end
...
return EquipmentInstancePauseCtrl
