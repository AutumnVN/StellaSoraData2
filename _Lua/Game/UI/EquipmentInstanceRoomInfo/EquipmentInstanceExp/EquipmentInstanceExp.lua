...
EquipmentInstanceExp._mapEventConfig = {}
local tbDescName = {
	[1] = "ThreeStarDesc",
	[2] = "TwoStarDesc",
	[3] = "OneStarDesc"
}
function EquipmentInstanceExp:Awake()
	self.AnimatorRoot = self.gameObject:GetComponent("Animator")
end
function EquipmentInstanceExp:FadeIn()
end
function EquipmentInstanceExp:FadeOut()
end
function EquipmentInstanceExp:OnEnable()
end
function EquipmentInstanceExp:OnDisable()
end
function EquipmentInstanceExp:OnDestroy()
...
function EquipmentInstanceExp:OnRelease()
end
function EquipmentInstanceExp:SetTime(nTime)
...
function EquipmentInstanceExp:StartEvent(nFloorId, nWaitTime, nLevelId)
	local mapFloorCfgData = ConfigTable.GetData("CharGemInstanceFloor", nFloorId)
	self.mapLevelCfgData = ConfigTable.GetData("CharGemInstance", nLevelId)
	self.tbTime = {
...
function EquipmentInstanceExp:OnEvent_SpecialMode_Count(nTime)
...
function EquipmentInstanceExp:StageChange()
	if self.bEnd then
		return
	end
...
function EquipmentInstanceExp:Success()
	self._mapNode.TMPDesc.gameObject:SetActive(false)
	self.bEnd = true
...
function EquipmentInstanceExp:ShowRtChallengeTime()
	self._mapNode.rtChallengeTime:SetActive(true)
end
function EquipmentInstanceExp:LevelEnd()
	EventManager.Remove("Equipment_Instance_Gameplay_Time", self, self.OnEvent_SpecialMode_Count)
	EventManager.Remove("OnEvent_ShowRtChallengeTime", self, self.ShowRtChallengeTime)
