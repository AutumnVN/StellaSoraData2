...
function ActivityLevelsSelectCtrl:RefreshInstanceInfo(nType, nHard, bLocation, bSetTog)
	...
	if self.tmpLockInfoDay ~= nil then
		NovaAPI.SetImageColor(self.tmpLockInfoDay, colorUnSelect)
	...
	elseif not isOpen then
		...
		if day == 0 then
			...
			NovaAPI.SetTMPText(self._mapNode.UnlockTime, orderedFormat(ConfigTable.GetUIText("ActivityLevels_Lock_DayHourOpen"), _day, _hour))
			...
			NovaAPI.SetImageColor(self.tmpLockInfoDay, colorSelect)
		end
	end
	for i = 1, #self.tabRewardList do
