local MiracleTaskCtrl = class("MiracleTaskCtrl", BaseCtrl)
...
MiracleTaskCtrl._mapNodeConfig = {
	TopBarPanel = {
...
function MiracleTaskCtrl:BuildData(nActivityId)
...
function MiracleTaskCtrl:onGridRefresh_Tab(go)
...
function MiracleTaskCtrl:onGridBtnClick_Tab(go)
	local nIndex = tonumber(go.name) + 1
...
function MiracleTaskCtrl:RefreshScaleOnClick_State(tr, nIndex, mapCfgData_ActivityTaskGroup, sProgress)
...
function MiracleTaskCtrl:refresh_Task(bPlayAnim)
...
function MiracleTaskCtrl:onGridRefresh_Task(go)
	local nIndex = tonumber(go.name) + 1
	local mapData = self.tbData[self.nCurGroupIndex]
...
function MiracleTaskCtrl:onEvent_ClickTaskJump(goBtn)
	local nJumpId = tonumber(goBtn.transform:GetChild(0).name)
	if 0 < nJumpId then
		JumpUtil.JumpTo(nJumpId)
	end
end
function MiracleTaskCtrl:refresh_Group()
	local mapData = self.tbData[self.nCurGroupIndex]
...
function MiracleTaskCtrl:onGridRefresh_GroupRewardItem(go)
	local nIndex = tonumber(go.name) + 1
...
function MiracleTaskCtrl:onGridBtnClick_GroupRewardItem(go)
...
function MiracleTaskCtrl:onBtn_GroupDone()
	local mapData = self.tbData[self.nCurGroupIndex]
	local cb = function()
		self:BuildData(self.nActivityId)
		self:refresh_Tab()
