...
local listPrefabPath = "UI/Play_Mining_400003/GridCell/"
local assetPath = "UI/Play_Mining_400003/SpriteAtlas/Sprite/"
...
function MiningGameCtrl:Awake()
	...
	if self.miningData:GetIsFirstIn() then
		local cb = function()
			local DicConfig = ConfigTable.GetData("TopBar", "MiningGame_400003")
...
function MiningGameCtrl:UpdateLevelData()
	...
	if self.gridListCtrl == nil then
		...
		self.gridListCtrl = self:BindCtrlByNode(self._mapNode.GirdListPrefab, "Game.UI.Play_Mining.400003.MiningGridListCtrl")
...
function MiningGameCtrl:OnBtnClick_OpenGuidePanel()
	EventManager.Hit(EventId.OpenPanel, PanelId.MiningGameGuidePanel_400003, self.nActId)
end
