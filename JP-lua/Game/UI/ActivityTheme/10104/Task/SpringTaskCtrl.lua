local SpringTaskCtrl = class("SpringTaskCtrl", BaseCtrl)
...
function SpringTaskCtrl:BuildData(nActivityId)
...
function SpringTaskCtrl:onGridRefresh_Tab(go)
...
function SpringTaskCtrl:onGridBtnClick_Tab(go)
	local nIndex = tonumber(go.name) + 1
...
function SpringTaskCtrl:RefreshScaleOnClick_State(tr, nIndex, mapCfgData_ActivityTaskGroup, sProgress)
	local canvasGroupOn = tr:Find("scale_on_click/imgDb_on"):GetComponent("CanvasGroup")
	local canvasGroupOff = tr:Find("scale_on_click/imgDb_off"):GetComponent("CanvasGroup")
	if self.nCurGroupIndex == nIndex then
...
function SpringTaskCtrl:refresh_Task(bPlayAnim)
...
function SpringTaskCtrl:onGridRefresh_Task(go)
	local nIndex = tonumber(go.name) + 1
	local mapData = self.tbData[self.nCurGroupIndex]
...
function SpringTaskCtrl:onEvent_ClickTaskJump(goBtn)
	local nJumpId = tonumber(goBtn.transform:GetChild(0).name)
	if 0 < nJumpId then
		JumpUtil.JumpTo(nJumpId)
	end
end
function SpringTaskCtrl:refresh_Group()
	local mapData = self.tbData[self.nCurGroupIndex]
...
function SpringTaskCtrl:onGridRefresh_GroupRewardItem(go)
	local nIndex = tonumber(go.name) + 1
...
function SpringTaskCtrl:onGridBtnClick_GroupRewardItem(go)
...
function SpringTaskCtrl:onBtn_GroupDone()
	local mapData = self.tbData[self.nCurGroupIndex]
	local cb = function()
		self:BuildData(self.nActivityId)
		self:refresh_Tab()
