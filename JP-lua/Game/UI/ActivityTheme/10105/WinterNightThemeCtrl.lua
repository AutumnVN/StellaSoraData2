...
function WinterNightThemeCtrl:Awake()
	local param = self:GetPanelParam()
	if type(param) == "table" then
		self.nActId = param[1]
	end
	self.WinterNightData = PlayerData.Activity:GetActivityGroupDataById(self.nActId)
...
function WinterNightThemeCtrl:OnEnable()
	self:RefreshPanel()
	for i = 1, 5 do
		local actData = self.WinterNightData:GetActivityDataByIndex(i)
...
function WinterNightThemeCtrl:OnDisable()
...
function WinterNightThemeCtrl:RefreshPanel()
	if self.WinterNightData == nil or self.ActivityGroupCfg == nil then
		return
	end
	self:RefreshTime()
...
function WinterNightThemeCtrl:RefreshTime()
	...
	local nOpenMonth, nOpenDay, nEndMonth, nEndDay, nOpenYear, nEndYear = self.WinterNightData:GetActGroupDate()
...
function WinterNightThemeCtrl:RefreshButtonState()
	self.tbActState = {}
	for i = 1, 5 do
		local actData = self.WinterNightData:GetActivityDataByIndex(i)
...
function WinterNightThemeCtrl:RefreshButtonTimer(actData, timer, txtTrans, imgTrans, refreshFunc)
	...
	if activityData ~= nil then
		...
		elseif state == ActivityState.Open and activityData.StartTime ~= "" and activityData.EndTime ~= "" then
			do
				local endTime = CS.ClientManager.Instance:ISO8601StrToTimeStamp(activityData.EndTime)
				if endTime > self.WinterNightData:GetActGroupEndTime() then
					bShowCountDown = true
				elseif endTime < self.WinterNightData:GetActGroupEndTime() then
...
function WinterNightThemeCtrl:RefreshMiniGameButtonState(actData)
	local activityId = actData.ActivityId
	local activityData = ConfigTable.GetData("Activity", activityId)
	if activityData ~= nil then
...
function WinterNightThemeCtrl:RefreshTaskButtonState(actData)
	local activityId = actData.ActivityId
...
function WinterNightThemeCtrl:RefreshStoryButtonState(actData)
	local activityId = actData.ActivityId
	local activityData = ConfigTable.GetData("Activity", activityId)
	if activityData ~= nil then
		local refreshFunc = function(actData)
...
function WinterNightThemeCtrl:RefreshLevelButtonState(actData)
	local activityId = actData.ActivityId
	local activityData = ConfigTable.GetData("Activity", activityId)
	if activityData ~= nil then
		local refreshFunc = function(actData)
...
function WinterNightThemeCtrl:RefreshShopButtonState(actData)
	local activityId = actData.ActivityId
	local activityData = ConfigTable.GetData("Activity", activityId)
	if activityData ~= nil then
		local refreshFunc = function(actData)
...
function WinterNightThemeCtrl:RequireActiviyData()
...
function WinterNightThemeCtrl:RefreshActivityData()
	if self.bRequiredActData then
		return
	end
	self:AddTimer(1, 3, self.RequireActiviyData, true, true, true)
end
function WinterNightThemeCtrl:OnBtn_ClickActivityEntrance(btn, nIndex)
	local actData = self.WinterNightData:GetActivityDataByIndex(nIndex)
