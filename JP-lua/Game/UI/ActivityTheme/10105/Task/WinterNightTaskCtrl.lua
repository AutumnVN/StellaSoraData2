local WinterNightTaskCtrl = class("WinterNightTaskCtrl", BaseCtrl)
...
WinterNightTaskCtrl._mapNodeConfig = {
	TopBarPanel = {
...
function WinterNightTaskCtrl:BuildData(nActivityId)
...
function WinterNightTaskCtrl:onGridRefresh_Tab(go)
...
function WinterNightTaskCtrl:onGridBtnClick_Tab(go)
	local nIndex = tonumber(go.name) + 1
...
function WinterNightTaskCtrl:RefreshScaleOnClick_State(tr, nIndex, mapCfgData_ActivityTaskGroup, sProgress)
	local canvasGroupOn = tr:Find("scale_on_click/imgDb_on"):GetComponent("CanvasGroup")
	local canvasGroupOff = tr:Find("scale_on_click/imgDb_off"):GetComponent("CanvasGroup")
	if self.nCurGroupIndex == nIndex then
...
function WinterNightTaskCtrl:refresh_Task(bPlayAnim)
...
function WinterNightTaskCtrl:onGridRefresh_Task(go)
	local nIndex = tonumber(go.name) + 1
	local mapData = self.tbData[self.nCurGroupIndex]
...
function WinterNightTaskCtrl:onEvent_ClickTaskJump(goBtn)
	local nJumpId = tonumber(goBtn.transform:GetChild(0).name)
	if 0 < nJumpId then
		JumpUtil.JumpTo(nJumpId)
	end
end
function WinterNightTaskCtrl:refresh_Group()
	local mapData = self.tbData[self.nCurGroupIndex]
...
function WinterNightTaskCtrl:onGridRefresh_GroupRewardItem(go)
	local nIndex = tonumber(go.name) + 1
...
function WinterNightTaskCtrl:onGridBtnClick_GroupRewardItem(go)
...
function WinterNightTaskCtrl:onBtn_GroupDone()
	local mapData = self.tbData[self.nCurGroupIndex]
	local cb = function()
		self:BuildData(self.nActivityId)
		self:refresh_Tab()
